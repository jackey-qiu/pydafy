...
class NewProject(QDialog):
    def __init__(self, parent=None):
        ...
        self.pushButton_ok.clicked.connect(lambda:self._creat_a_new_project( parent = parent,
                                                                            name_db = 'beamtime_'+self.lineEdit_name.text(), 
    ...
    def _creat_a_new_project(self,parent, name_db, db_info,type_db ='paper'):
        ...
        beamtime_info = {'project_info':db_info,
        ...
        if type_db == 'paper':
            parent.database.project_info.insert_one(beamtime_info)
        parent.comboBox_project_list.clear()
        parent.comboBox_project_list.addItems([each for each in parent.mongo_client.list_database_names() if each.startswith('beamtime')])
        parent.comboBox_project_list.setCurrentText(name_db)
        #extract project info
        all = parent.database.project_info.find()
        db.load_project(parent)
...
class LoginDialog(QDialog):
    ...
    def login(self, parent, url):
        ...
        try:
            ...
            parent.statusLabel.setText('Welcome {}, you are logged in to MangoDB Atlas system as a {}!'.format(name, parent.role))
            self.close()
